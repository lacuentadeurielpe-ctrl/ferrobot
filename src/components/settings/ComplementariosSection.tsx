...
export default function ComplementariosSection({ productos }: Props) {
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h2 className="text-sm font-semibold text-zinc-700">Productos complementarios</h2>
      ...
      <div className="bg-zinc-50 rounded-xl border border-zinc-200 p-4 space-y-3">
        <p className="text-xs font-medium text-zinc-600">Agregar par manual</p>
        ...
        <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-2">
          <div className="relative">
            <select
              ...
              className="w-full text-sm border border-zinc-300 rounded-xl px-3 py-2 pr-8 appearance-none bg-white focus:outline-none focus:ring-2 focus:ring-zinc-300"
            >
      ...
      {cargando && <p className="text-xs text-zinc-400">Cargando…</p>}
      ...
      {!cargando && manuales.length > 0 && (
        <div>
          <div className="flex items-center gap-1.5 mb-2">
            ...
            <span className="text-xs font-semibold text-zinc-600">Configurados por ti ({manuales.length})</span>
          </div>
            ...
            {manuales.map((par) => (
              <div
                ...
                className={`flex items-center justify-between px-3 py-2 rounded-xl border text-sm ${
                  par.activo ? 'bg-white border-zinc-200' : 'bg-zinc-50 border-zinc-100 opacity-60'
                }`}
              >
                <div className="flex items-center gap-2 min-w-0">
                  <span className="font-medium text-zinc-800 truncate">{par.producto.nombre}</span>
                  <span className="text-zinc-400 text-xs shrink-0">→</span>
                  <span className="text-zinc-700 truncate">{par.complementario.nombre}</span>
                </div>
          ...
          <div className="space-y-1">
            {autos.map((par) => (
              <div
                key={par.id}
                className={`flex items-center justify-between px-3 py-2 rounded-xl border text-sm ${
                  par.activo ? 'bg-blue-50 border-blue-100' : 'bg-zinc-50 border-zinc-100 opacity-60'
                }`}
              >
                <div className="flex items-center gap-2 min-w-0">
                  <span className="font-medium text-zinc-800 truncate">{par.producto.nombre}</span>
